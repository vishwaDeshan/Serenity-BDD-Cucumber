Feature: Add a Book in the Library
  ...
  Scenario: Add a Book with a Negative ID
    ...
    Then the response status code should be 201
    And the response should contain the added book details with a valid auto-generated id, title "The Book Negative ID" and author "Author Negative ID"
  ...
  Scenario: Add a Book with an Existing ID but Different Title and Author
    ...
    Then the response status code should be 400
    And the response should contain error "The book ID already exists"
  ...
  Scenario: Add a book with admin
    Given I have admin credentials
    When I add a book with title "Admin Book" and author "Admin Author"
    Then the response status code should be 201
    And the response should contain the added book details with title "Admin Book" and author "Admin Author"
